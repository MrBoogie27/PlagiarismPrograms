...
public:
    virtual std::unique_ptr<clang::ASTConsumer> CreateASTConsumer(
        ...
        return std::make_unique<ASTConsumerHasher>(&Compiler.getASTContext());
    }
};
...
void dfs(Decl* decl) {
    ...
    if (decl->getKind() == Decl::Var) {
        auto declVar = static_cast<VarDecl*>(decl);
        std::cout << "DeclVar: ";
        declVar->dump();
    }
...
//    int result = Tool.run(newFrontendActionFactory<PrintAllAST>().get());
    ...
    Tool.buildASTs(ASTs);
    std::cout << ASTs.size() << std::endl;
    auto root = ASTs[0]->getASTContext().getTranslationUnitDecl();
    for (auto D = root->decls_begin(); D != root->decls_end(); D++) {
        dfs(*D);
    }
