...
namespace BearTrap {
    ...
    public:
        VisitorButtomUpMatcherBase(ASTContext *Context, Node2NodeMap &matchers)
                : Context(Context), Matchers(matchers) {
        }
...
#undef VISIT
    ...
    public:
        VisitorButtomUpSecondMatcher(ASTContext *Context,
                                     ...
                                     size_t countDesc,
                                     Node2NodeMap &matchers)
                : VisitorButtomUpMatcherBase(Context, matchers), OriginalNode(node),
                  NodeChildMatch(std::move(node_child_match)), NodeCountDesc(countDesc), BestNode({0, 0}) {
    ...
    public:
        explicit ASTConsumerButtomUpSecondMatcher(ASTContext *Context,
                                                  ...
                                                  size_t countDesc,
                                                  Node2NodeMap &matchers)
                : Visitor(Context, node, std::move(node_child_match), countDesc, matchers), Matchers(matchers) {}
    ...
    public:
        VisitorButtomUpFirstMatcher(ASTContext *Context, ASTContext *Context2, Node2NodeMap &matchers)
                : VisitorButtomUpMatcherBase(Context, matchers), Context2(Context2) {
        }
